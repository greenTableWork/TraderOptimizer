...
from trader_optimizer.volatility_regime import (
    DEFAULT_VOLATILITY_REGIME_QUANTILES,
    build_volatility_regime_config,
    write_volatility_regime_config,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = build_volatility_regime_config(
        symbol_bars,
        periods=periods,
        bucket_timezone=args.bucket_timezone,
        min_bars=args.min_bars,
        quantiles=tuple(args.quantile or DEFAULT_VOLATILITY_REGIME_QUANTILES),
        profiles=profiles,
        start_utc=args.start_utc,
        end_utc=args.end_utc,
    )
    ...
    output_path = (
        ...
        / "runs"
        / "tuning_regions"
        / f"{timestamp}_volatility_regime_config.json"
    ).resolve()
    write_volatility_regime_config(output_path, config)
    if not args.quiet:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build per-instrument volatility regime thresholds from PostgreSQL "
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Minimum bars required for a period bucket to become a volatility sample.",
    )
    parser.add_argument(
        "--quantile",
        type=float,
        ...
        default=None,
        help=(
            "Regime boundary quantile. Pass exactly two values. Defaults to "
